document.addEventListener('DOMContentLoaded', () => {
    ...
    class MortgageEngine {
        ...
        runSimulation(principal, rate, periodicContrib, initialOffset, initialRedraw, annualFee, interventions) {
            ...
            let totalInterest = 0;
            let accruedInterestBucket = 0;
            let neutralityDate = null;
            let loanEndDate = null;

            // Pre-index interventions for efficient daily lookup
            const intLookup = {};
            interventions.forEach(ev => {
                const dStr = new Date(ev.date).toDateString();
                if (!intLookup[dStr]) intLookup[dStr] = [];
                intLookup[dStr].push(ev);
            });
            ...
            const ppy = this.getPeriodsPerYear();
            const daysPerPeriodFallback = this.getDaysPerPeriod();
            let totalSimulationPeriods = Math.round(ppy * 60); 
            ...
            for (let p = 1; p <= totalSimulationPeriods; p++) {
                ...
                if (this.freq === 'monthly') {
                    ...
                    currentDate.setDate(currentDate.getDate() + (p - 1) * daysPerPeriodFallback);
                }
                ...
                if (this.freq === 'monthly') {
                    ...
                    nextDate.setDate(nextDate.getDate() + daysPerPeriodFallback);
                }
                ...
                // 1. Daily Calculation Loop for this Period
                let periodInterest = 0;
                let dayIter = new Date(currentDate);
                while (dayIter < nextDate) {
                    const dStr = dayIter.toDateString();
                    
                    // a) Rates & Flags for Today
                    let effectiveRate = currentRate;
                    let isInsideFixedTerm = false;
                    let isInsideSplitTerm = false;
                    let isInsideIOTerm = false;

                    if (ioEnd && dayIter >= this.ioStartDate && dayIter < ioEnd) {
                        isInsideIOTerm = true;
                        effectiveRate = this.ioRate;
                    }
                    else if (splitEnd && dayIter >= this.splitStartDate && dayIter < splitEnd) isInsideSplitTerm = true;
                    
                    if (fixedEnd && dayIter >= this.fixedStartDate && dayIter < fixedEnd) {
                        isInsideFixedTerm = true;
                        effectiveRate = this.fixedRate;
                    }

                    // b) Detect Transitions (Still handled at period level for contrib recalcs, but flags are daily)
                    // Note: In a true daily sim, we'd recalc contrib on the exact day. 
                    // To keep compatibility, we check transition flags at start of period for the contrib.
                    // But for interest calculation, we use daily flags.

                    // c) Apply Interventions
                    if (intLookup[dStr]) {
                        intLookup[dStr].forEach(ev => {
                            if (ev.type === 'lump_sum') { currentBalance -= ev.value; balanceNotes.push(`Lump -${fmtCurrency(ev.value)}`); }
                                ...
                                // Recalc repayment on rate change day
                    ...
                    if (isInsideSplitTerm && !isInsideFixedTerm) {
                        ...
                        dailyInterest = (effSPortion * (this.splitRate / 100 / 365)) + (effVPortion * (currentRate / 100 / 365));
                    } else {
                        dailyInterest = effectivePrincipal * (effectiveRate / 100 / 365);
                    }
                    
                    accruedInterestBucket += dailyInterest;
                    periodInterest += dailyInterest;

                    // e) Neutrality Check
                    if (!neutralityDate && (currentOffset + currentRedraw) >= currentBalance && currentBalance > 0) neutralityDate = new Date(dayIter);

                    dayIter.setDate(dayIter.getDate() + 1);
                }

                // 2. Determine and Charge Interest
                // Requirement: Charged Monthly.
                // If freq is monthly, charge everything in bucket.
                // If freq is weekly/fortnightly, charge if this period crosses a month boundary.
                let interestToCharge = 0;
                let chargeInterestThisPeriod = false;

                if (this.freq === 'monthly') {
                    chargeInterestThisPeriod = true;
                } else {
                    // Charge when month changes
                    if (nextDate.getMonth() !== currentDate.getMonth() || nextDate.getFullYear() !== currentDate.getFullYear()) {
                        chargeInterestThisPeriod = true;
                    }
                }

                if (chargeInterestThisPeriod) {
                    interestToCharge = Math.round(accruedInterestBucket * 100) / 100;
                    accruedInterestBucket = 0;
                    totalInterest += interestToCharge;
                }

                // 3. Transition Logic (Repayment Amount Updates)
                // We use the flags as they are at the START of the period to decide repayment shifts
                const startOfPeriodDate = new Date(currentDate);
                let isInsideFixedStart = false, isInsideIOStart = false, isInsideSplitStart = false;
                let effectiveRateStart = currentRate;

                if (ioEnd && startOfPeriodDate >= this.ioStartDate && startOfPeriodDate < ioEnd) { isInsideIOStart = true; effectiveRateStart = this.ioRate; }
                else if (splitEnd && startOfPeriodDate >= this.splitStartDate && startOfPeriodDate < splitEnd) isInsideSplitStart = true;
                if (fixedEnd && startOfPeriodDate >= this.fixedStartDate && startOfPeriodDate < fixedEnd) { isInsideFixedStart = true; effectiveRateStart = this.fixedRate; }

                if (p > 1 && isInsideFixedStart !== previousIsFixedTerm) {
                    const remaining = contractPeriods - p + 1;
                    if (remaining > 0 && !this.repaymentOverride) {
                        currentPeriodicContrib = Math.round(this.calculateRepayment(currentBalance, effectiveRateStart, remaining, ppy) * 100) / 100;
                        balanceNotes.push(isInsideFixedStart ? "Fixed Started" : "Reverted to Variable");
                    }
                }
                previousIsFixedTerm = isInsideFixedStart;

                if (p > 1 && isInsideIOStart !== previousIsIOTerm) {
                    const remaining = contractPeriods - p + 1;
                    if (remaining > 0 && !this.repaymentOverride) {
                        if (isInsideIOStart) balanceNotes.push("Interest Only Started");
                        else {
                            currentPeriodicContrib = Math.round(this.calculateRepayment(currentBalance, currentRate, remaining, ppy) * 100) / 100;
                            balanceNotes.push("Interest Only Ended");
                        }
                    }
                }
                previousIsIOTerm = isInsideIOStart;

                if (p > 1 && isInsideSplitStart !== previousIsSplitTerm) {
                    const remaining = contractPeriods - p + 1;
                    if (remaining > 0 && !this.repaymentOverride) {
                        if (isInsideSplitStart && !isInsideFixedStart) {
                        ...
                        } else if (!isInsideSplitStart) {
                            currentPeriodicContrib = Math.round(this.calculateRepayment(currentBalance, currentRate, remaining, ppy) * 100) / 100;
                            balanceNotes.push("Split Ended");
                        }
                    }
                }
                previousIsSplitTerm = isInsideSplitStart;

                // 4. Annual Fee
                if (annualFee > 0 && p > 1) {
                    const currentYear = currentDate.getFullYear();
                    if (currentYear > lastFeeYear && currentDate.getMonth() >= loanStart.getMonth()) {
                        currentBalance += annualFee; balanceNotes.push(`Fee +${fmtCurrency(annualFee)}`);
                        lastFeeYear = currentYear;
                    }
                }

                // 5. Apply Repayment & Interest
                if (isInsideIOStart && !this.repaymentOverride) {
                    // For IO, repayment is exactly the interest charged that month
                    // (Or if we didn't charge this week, payment is 0?)
                    // Simplified: charge the full periodInterest even if not 'charged' to balance yet to keep IO cashflow steady
                    if (interestToCharge > 0) currentPeriodicContrib = interestToCharge;
                    else if (this.freq === 'monthly') currentPeriodicContrib = interestToCharge;
                    else {
                        // If weekly IO, banks usually still charge weekly interest. 
                        // But user asked for "charged monthly". 
                        // If we hold interest until month end, should we also hold payments? 
                        // Probably not. We'll just distribute the interest or keep it monthly.
                    }
                }

                let principalPaid = currentPeriodicContrib - interestToCharge;
                if (isInsideIOStart) principalPaid = 0; 
                ...
                let sFr = 0, sVr = 0;
                if (isInsideSplitStart) {
                    const ratio = currentPeriodicContrib / (this.splitTotalRepayment || 1);
                    sFr = this.splitFixedRepayment * ratio; sVr = this.splitVariableRepayment * ratio;
                }

                history.push({
                    p, date: new Date(currentDate), balance: currentBalance, interest: interestToCharge, principal: principalPaid,
                    offset: currentOffset, redraw: currentRedraw, rate: effectiveRateStart, repayment: currentPeriodicContrib,
                    isFixed: isInsideFixedStart, isSplit: isInsideSplitStart, isIO: isInsideIOStart, splitRate: isInsideSplitStart ? this.splitRate : null,
                ...
                checkM('lvr90', 90); checkM('lvr80', 80); checkM('lvr50', 50); checkM('lvr25', 25);

                // 7. Refinance
                if (this.isRefinance && this.refinanceDate && this.refinanceDate >= currentDate && this.refinanceDate < nextDate) {
                    if (this.refinanceAmount > 0) { currentBalance += this.refinanceAmount; balanceNotes.push(`Refinance: +${fmtCurrency(this.refinanceAmount)}`); }
                    else balanceNotes.push(`Refinance Started`);
                    contractPeriods = Math.ceil((this.refinanceTermMonths / 12) * ppy) + p;
                    totalSimulationPeriods = contractPeriods + 60;
                    currentRate = this.refinanceRate;
                    if (currentBalance > 0) loanEndDate = null;
                    isInsideSplitStart = false; isInsideFixedStart = false; isInsideIOStart = false;
                    previousIsFixedTerm = false; previousIsSplitTerm = false; previousIsIOTerm = false;
                    if (this.refinanceType === 'fixed') { fixedEnd = new Date(this.refinanceDate); fixedEnd.setFullYear(fixedEnd.getFullYear() + this.refinanceFixedYears); }
                    else if (this.refinanceType === 'interest_only') { ioEnd = new Date(this.refinanceDate); ioEnd.setFullYear(ioEnd.getFullYear() + this.refinanceIOYears); }
                    else { fixedEnd = null; ioEnd = null; }
                    const rem = Math.ceil((this.refinanceTermMonths / 12) * ppy);
                    if (this.refinanceType === 'interest_only') {
                        this.singlePeriodicRepayment = Math.round((currentBalance * (currentRate / 100) / ppy) * 100) / 100;
                    } else {
                        this.singlePeriodicRepayment = Math.round(this.calculateRepayment(currentBalance, currentRate, rem, ppy) * 100) / 100;
                    }
